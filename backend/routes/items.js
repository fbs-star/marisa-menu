...
router.get('/', requireAuth, async (req, res) => {
  ...
  let rows;
  if (category_id) {
    rows = await db.all('SELECT * FROM items WHERE category_id = ? ORDER BY sort_order ASC, id ASC', [category_id]);
  } else {
    rows = await db.all('SELECT * FROM items ORDER BY category_id ASC, sort_order ASC, id ASC');
  }
...
router.post('/', requireAuth, async (req, res) => {
  ...
  const maxOrderRow = await db.get('SELECT MAX(sort_order) AS m FROM items WHERE category_id = ?', [body.category_id]);
...
router.patch('/:id/toggle-sold-out', requireAuth, async (req, res) => {
  const row = await db.get('SELECT * FROM items WHERE id = ?', [req.params.id]);
  if (!row) return res.status(404).json({ error: 'Not found' });
  const next = row.is_snooze ? 0 : 1;
  await db.run('UPDATE items SET is_snooze=?, updated_at=CURRENT_TIMESTAMP WHERE id=?', [next, req.params.id]);
...
router.patch('/:id/toggle-published', requireAuth, async (req, res) => {
  ...
  await db.run('UPDATE items SET published=?, updated_at=CURRENT_TIMESTAMP WHERE id=?', [next, req.params.id]);
