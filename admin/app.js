(function () {
  ...
  let state = { user: null, page: 'categories', categories: [], items: [], activeCategoryId: null };
  ...
  async function loadAll() {
    const [categories, items] = await Promise.all([api('/admin/categories'), api('/admin/items')]);
  ...
  function renderCategoriesPage() {
    ...
    main.innerHTML = `
      ...
      <div class="card">
        <table>
          <thead><tr><th>Name (EN)</th><th>Name (TH)</th><th>Items</th><th>Status</th><th></th></tr></thead>
          <tbody>
            ...
            `).join('') || `<tr><td colspan="5" class="empty-hint">No categories yet.</td></tr>`}
          </tbody>
        </table>
      </div>
    `;
  ...
  function openCategoryModal(category) {
    ...
    const data = category || { name: {}, description: {}, is_new: false, is_signature: false };
